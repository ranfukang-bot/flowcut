...
export async function POST(request: Request) {
  try {
    ...
    if (!sessionToken || fingerprintHash.length < 32) {
      return Response.json({ error: "授权会话或设备信息缺失" }, { status: 400 });
    }
    ...
    if (session.fingerprint_hash !== fingerprintHash || session.device_status !== "active") {
    ...
    return Response.json(
      {
        ok: true,
        lease: await issueLease(session, session.device_id),
        account: {
          username: session.username,
          expiresAt: session.expires_at,
          planCode: session.plan_code,
          offlineGraceHours: session.offline_grace_hours,
    ...
    await audit("desktop", "lease_error", "", {
