...
const STATE_DIR = path.join(process.env.FLOWCUT_PUBLISHER_DATA_DIR || path.join(__dirname, '..'), 'state');
...
export function setState(accountName, state) {
  ...
  // Preserve confirmed workflow publications when completed files are later pruned from the scanner.
  state.publishedFiles ??= {};
  for (let i = 0; i <= (state.doneIndex ?? -1); i++) {
    const item = state.items?.[i];
    if (item?.relativePath && /^[a-f0-9-]{36}\/\d+\.mp4$/.test(item.relativePath)) state.publishedFiles[item.relativePath] ??= state.updatedAt;
  }
  const file = statePath(accountName);
